...
class FWBTagEfficiencyOnBDT {
    public:
        ...
        double get(const LorentzVector& jet1, const LorentzVector& jet2, float BDTout) {
            ...
            for (const auto& flav1: flavors) {
                ...
                for (const auto& flav2: flavors) {
                    ...
                    const auto& fraction_eff = m_fractions[std::make_tuple(flav1, flav2)];
                    auto fraction = fraction_eff->GetEfficiency(fraction_eff->FindFixBin(BDTout));
        ...
        FWBTagEfficiencyOnBDT(const std::string& btag_efficiencies, const std::string& flavor_fraction) {
            ...
            for (const auto& flav1: flavors) {
                for (const auto& flav2: flavors) {
                    ...
                    std::tuple<uint8_t, uint8_t> key = std::make_tuple(flav1, flav2);
    ...
    private:
        ...
        using key = std::tuple<uint8_t, uint8_t>;
